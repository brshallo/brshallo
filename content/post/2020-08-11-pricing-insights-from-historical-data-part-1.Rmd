...
* Even if you plan on using a linear model, using a generic more complex machine learning model can be helpful as a sanity check. If model performance is not hugely different from your linear model, you are fine, if it is substantially different that may mean there is some important relationship you are missing and need to identify.
