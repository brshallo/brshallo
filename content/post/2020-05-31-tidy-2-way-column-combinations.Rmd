...
First revert to a form similar to where we were at the end of [I. Nest and pivot] where we had two columns:
...
Let's use the new `operations_combinations()` function to create new columns for the differences and quotients between all pairwise combinations of our features of interest.
```{r}
...
Perhaps you want to calculate some measure of association between your new features and `Sale_price`. To keep things simple, I'm going to first remove any columns that contain any NA's or infinite values. I'll also use a random sample of just 1500 rows.
```{r}
...
Specifically, maybe you want to see the statistical significance of the correlation of each feature with `Sale_Price`. I'll calculate these across variables and then plot them on a histogram.
```{r}
select(ames_sp_features_subset, -Sale_Price) %>% 
...
If doing predictive modeling or inference you probably want to fit any computation and analysis into a `tidymodels` process or other framework. For some brief notes on this see [Interactions example, tidymodels].
