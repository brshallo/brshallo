...
All types of games described in this post would need to be empirically evaluated in a gambling / gaming environment. The ideas are very much in a draft form^[There is a decent chance I misapplied some concepts -- particularly regarding the idea of risk seeking when it comes to losses (e.g. if it's more about seeking uncertainty vs seeking riskiness...). What I describe also seems to run counter to the idea that sporadic positive events help drive repeat behaviors whereas sporadic negative shocks drive avoidance of the activity. Generally I would need to think about and read a little more...]. The games discussed align with some psychological biases while potentially running counter to others^[A potential issue with my examples is with the 'framing effect' -- I largely frame things in terms of 'losses' (particularly in the [Wallet Roulette] game which may actually push players towards more risk-averse behavior. For example people's tendency to underweight rare events and a closer consideration of the importance of reference points are just a few ideas that require closer consideration.] -- reading more on [prospect theory](https://en.wikipedia.org/wiki/Prospect_theory) and related ideas is required to more carefully consider approaches in game design[^5].

# Note on Prospect Theory

*Section added after publishing post and after reading a little about Prospect Theory. You may want to skip this section and come back to it at the end.*

The course materials I reference in this post are themselves referencing research on when a player is facing *either* only gains (gauranteed gain vs possible gain) or only losses (gauranteed loss vs possible loss). However in gambling the problem is mixed. Players face both the possibility of gains *or* losses on any given bet. Khaneman describes the differences in the mixed case in *Chapter 26, Prospect Theory* of his book *Thinking Fast and Slow*:

> "In mixed gambles, where both a gain and a loss are possible, loss aversion causes extremely risk averse choices. In bad choices, where a sure loss is compared to a larger loss that is merely probable, diminishing sensitity causes risk seeking. There is no contradiction. In the mixed case the possible loss looms twice as large as the possible gain."

This suggests that my assertions throughout later parts of this post regarding risk seeking behaviors are likely incorrect. This *does not present* a problem for my references of people's preference for aggregated losses and disaggregated gains. 

It remains possible that multiple gains may represent greater value than a single loss. The chart below shows a typical example of the assymetric S-shaped value function associated with gains/losses in Prospect Theory. Even with this assymetry it is clear that three gains (i.e. wins) of \$0.05 would more than offset a single loss of \$0.15 (assuming the value derived from individual gains can be aggregated linearly). Hence the types of games I describe may still be worth exploring in gambling contexts.

![](https://upload.wikimedia.org/wikipedia/commons/thumb/8/85/Loss_Aversion.png/1024px-Loss_Aversion.png)
*(Image from Wikipedia page on [Prospect Theory](https://en.wikipedia.org/wiki/Prospect_theory))*

One explanation for why people gamble at all is people's tendency to overweight the likelihood of rare events^[I.e. they *think* they have a better chance of winning that they do.]. While this explains participation in lotteries it does not explain all forms of gambling. These other forms of gambling present additional challenges for Prospect Theory^[As well as other behavioral economic theories] to explain. Nick Barberis attempts to explain gambling in [A Model of Casino Gambling](http://www.econ.yale.edu/~shiller/behfin/2009_03/barberis.pdf) where he writes: 
...
*Below I will describe the rules for a game that I've come-up with called "Wallet Roulette^[or "Russian Wallet Roulette" or some variation on this.]" that is designed to take advantage of the ideas described in the post above (the game though does not reflect a complete embodiment of Prospect Theory applied to gambling^[See footnotes from beginning of post for notes on this... obviously though this game is framed as a loss, which may run counter to the type of game that a Prospect Theory agent may want to play.]). The game is set-up to look and feel somewhat similar to a slot machine but with a win / loss structure that is almost the [complement](https://en.wikipedia.org/wiki/Complementary_event) of that in slots^[I.e. the player typically has small wins followed by occasional large losses whereas slots tend to have small losses with occasionally larger wins. However in the "Game Options" I provide ways by which a player could set their own risk profile (e.g. by filling their gun with more bullets, changing the cylinder size, etc.). Hence I also envision the game having a bit more customizability compared to what is typical in slots.].*
...
The game begins when you take a wallet and fill it with money (your bet) and place it in front of you. A gun, a revolver, with six chambers in it is pointed at your wallet. The round starts when you fill one of the chambers with a bullet, spin the cylinder, wait for it to come to a rest, and pull the trigger. If the gun fires, your wallet explodes and the money flies out and is scooped up by the shady figures. If it clicks (having landed on an empty chamber) the attendant grimaces and slides some money to you from across the counter^[This gets added to your account or can be added to the wallet, i.e. your bet.]. You then decide whether to play another round (first filling a new wallet with money if your previous one had exploded) or you take out your money and leave.
