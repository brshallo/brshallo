...
However each of these examples were built using PowerPoint (and a lot of copy and pasting of code + output). The series of images cannot be easily reproduced (at least not without going through another highly manual exercise). In this post, I'll point to resources on how to create these sorts of code communication materials in ways that *are reproducible*.
...
The remainder of this post discusses either tools adjacent to flipbooks or describes workflows that can also be found within `flipbookr` documentation.
...
I sometimes want to convert a flipbook into a gif, e.g. when sharing an example in a README or a snippet of a concept on social media. If you ignored my prior entreaty, this is a second reminder to stop and go read about `flipbookr`. The [template file](https://evamaerey.github.io/flipbooks/about) now shows how to create gifs using `flipbookr` --> `pagedown` --> `magick`^[Creates html --> pdf --> gif.]. I also describe this workflow and provide examples [here](https://github.com/brshallo/flipbookr-gifs-examples), e.g.
...
For more sophisticated highlighting of *code*, use the [flair package](https://github.com/kbodwin/flair). I'm not sure what to recommend for highlighting changes in *output* to the console... perhaps [diffobj](https://github.com/brodieG/diffobj) would be an option. You could also just more explicitly format the output, e.g. using  [gt](https://github.com/rstudio/gt) or [kableExtra](https://github.com/haozhu233/kableExtra) for tabular outputs, or using geoms, annotations, etc. in [ggplot](https://github.com/tidyverse/ggplot2)s. And, of course, you can always dive into the html.
...
* While this post was in draft form, I tweeted out my initial approach (that used the [flair](https://github.com/kbodwin/flair) package). 
    * The next step might have been trying to improve upon this. Thankfully, instead, [Kelly Bodwin](https://twitter.com/KellyBodwin) pointed me to `flipbookr` `r emo::ji("smile")`! 
...
* I remade my initial example using `flipbookr` ([see issue](https://github.com/EvaMaeRey/flipbookr/issues/22)). 
    * I had first created an [issue](https://github.com/EvaMaeRey/flipbookr/issues/21) showing how to print `xaringan` slides incrementally using `pagedown::chrome_print()`.
        * Which helped to close the loop on a related open [issue](https://github.com/rstudio/pagedown/issues/110) on `xaringan`.
