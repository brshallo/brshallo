...
If you are creating a function that can take in multiple sets of columns, e.g. you want to `group_by()` one set of variables and `summarise()` another set, you could do:
...
You could also make the inputs to `.fns` an argument in your function^[Though doing this doesn't require any tidy evaluation knowledge.].

# Older approaches

The methods in this section still work and are supported, they are just no longer the 'recommended' or most 'modern' approach available for creating functions that pass column names into `dplyr` verbs. Generally, I think the new `across()` approaches introduced in `dplyr` 1.0.0 are easier and more consistent to use than the methods that preceded them.

Prior to the introduction of the **bracket-bracket**, `{{}}`, you would have used the **`enquo()` + bang-bang** approach^[There is also the `rlang::enquos()` and `!!!` operator for when the input has length greater than one.] for building a function that summarised one variable. E.g.
...
Similar to using `across()` this method allows multiple variables being input. However what is weird about this function is that it requires the user wrapping the variable names with the `vars()` function^[A niche function specific to tidy evaluation (which users might not think to use).]. E.g. to use the previously created function, a user would run:
...
One other older approach perhaps worth mentioning (presented [here](https://rstudio.com/resources/rstudioconf-2019/working-with-names-and-expressions-in-your-tidy-eval-code/)) is "passing the dots". Example if we want to `group_by()` multiple columns and then `summarise()` on just one column:
