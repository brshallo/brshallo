...
groupsum <- function(df, group_vars, sum_vars){
  df %>% 
    ...
    summarise_at(vars(one_of(sum_vars)), sum)
}
```
...
There are a variety of other spins you might take on creating these types of functions. One other older approach perhaps worth mentioning (presented [here](https://rstudio.com/resources/rstudioconf-2019/working-with-names-and-expressions-in-your-tidy-eval-code/)) was "passing the dots". 

Example if we want to `group_by()` multiple columns and then summarise on just one column:
