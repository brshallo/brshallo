...
A problem I bumped into was that most of Chelsea's gists don't actually have .R or .Rmd extensions so my approach skipped most of her snippets. I wanted to parse my own gists but ran into the similar problem that most of my github gist code snippets are in .md files[^As I often use this output type when creating a [reprex](https://github.com/tidyverse/reprex)).

In this post I...

1. create a function to extract code chunks from simple .md files
1. parse the functions and packages in my code using [funspotr](https://github.com/brshallo/funspotr)

In the future I may do a follow-up that passes the parsed functions and packages through a network analysis or some other to better visualize the relationships between code snippets.
...
* reads in a file
* extracts the text in code chunks^[based on what exists between ticks.]
...
gists_unnested <- bind_rows(
  ...
  # add back-in links to url's where files are rather than urls equalling local
  # paths for .md snippets
  select(-urls) %>% 
...
Perhaps I'll do a follow-up and show some ways the relationships between the resulting parsed code snippets may be visualized or organized. 

Mentioned in the initial thread, Obsidian seems to be a product that does some things along the lines of what I was thinking:
