...
```{r}
start_grid <- animatrixr::construct_grid(-12:12, -12:12) %>% 
...
house_points <- crossing(x = -5:5, y = -5:5) %>% 
...
p <- bind_rows(start_grid, end_grid) %>% 
  ...
  scale_x_continuous(breaks = -5L:5L, minor_breaks = NULL)+
  scale_y_continuous(breaks = -5L:5L, minor_breaks = NULL)+
