...
1. relying on fewer model assumptions (though still generally requires that your model's errors are [iid](https://en.wikipedia.org/wiki/Independent_and_identically_distributed_random_variables)) to still produce reasonable measures of uncertainty. 
...
The [Appendix] provides links to [Other Examples Using Simulation].
...
Simulation produced intervals show more differences in relative interval widths compared to those outputted with analytic methods: [Part 1, Review Prediction Intervals](https://www.bryanshalloway.com/2021/03/18/intuition-on-uncertainty-of-predictions-introduction-to-prediction-intervals/#review-prediction-intervals) (where the relative interval widths were nearly constant across predictions).

## Interval Width

The relative interval widths for the simulation based approach vary between observations by more than 10x what we saw in the interval widths using [Analytic Methods](http://localhost:4321/2021/03/18/intuition-on-uncertainty-of-predictions-introduction-to-prediction-intervals/#interval-width)[^differences] (where interval widths had a standard deviation of less than half a percentage point). We also see greater differences in interval width between buckets of predictions (a range of about 6.5 percentage points vs 0.4). 
...
We are also looking at just one model specification for one set of data... further examples would be needed to make more general statements on the efficacy of this simulation based approach...
...
This post walked through a rough implementation for simulating prediction intervals for arbitrary model types + preprocessing steps in a way that is loosely in the style of tidymodels.
