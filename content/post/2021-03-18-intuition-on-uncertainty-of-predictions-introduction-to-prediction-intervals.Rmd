...
However understanding the intuition on these sources of uncertainty in prediction intervals is still helpful. In follow-up posts I use more flexible methods for building prediction intervals. These other methods can produce substantive differences in the width of prediction intervals between observations^[For example, simulation techniques are able to capture uncertainty in model estimation in ways that do not depend on distance of an observation from the centroid of the data. They also capture uncertainty that results from trying to estimate too many parameters relative to the size of the dataset.].
...
lm_interval_widths %>% 
  gt::gt() %>% 
 gt::fmt_number(c("stdev", "stderror"), decimals = 4) %>% 
 gt::fmt_number("mean_interval_width_percentage", decimals = 2)
```
