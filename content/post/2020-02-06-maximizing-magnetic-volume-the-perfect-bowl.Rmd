...
This post contains solutions to FiveThirtyEight's two riddles released 2020-01-31, [Riddler Express] and [Riddler Classic].
...
![Finding pyramid height](/post/2020-02-06-maximizing-magnetic-volume-the-perfect-bowl_files/height_calc.jpg){width=600px}
...
Using the trigonemtric rule that *cos(angle) = adjacent/hypotenuse* reveals the length of a Magna-Tile as: *cos(15&deg;)* (Remember we are treating the longest sides of the Magna-Tile as equal to 1 unit^[In this case the longest sides are each a hypotenuse.].
...
**II.** I used Math Open Reference to find the equation for [apothem length](https://www.mathopenref.com/apothem.html) as a function of number and length of sides in a regular polygon. *apothem = (side length) / (2 x tan(180&deg; / number of sides))* . You can replace *(side length)* in this equation with *2 x sin(15&deg;)* (calculated above), making apothem length a function of *only* the number of sides on our pyramid (i.e. the number of Magna-Tiles).
...
**III.** Parts **I.** and **II.** represent two sides of a right triangle. To find the third side, corresponding with pyramid height, simpy use the pythagorean theorem:  *sqrt(magnatile_length^2 - base_apothem^2)*. Fill these in with the parts detailed in **I** & **II** above and you'll see pyramid height is now represented as a function of *only* # of sides (i.e. number of Magna-Tiles).
