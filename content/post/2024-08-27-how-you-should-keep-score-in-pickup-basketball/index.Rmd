...
If we consider the expected points of an interior shot (when playing 1s and 2s) the series can be represented as:
...
We can use this to determine, roughly, the expected points^[As I discuss in [Finite geometric series] in the appendix the value is dependent on how many points are left in the game, but in practice, this doesn't make a big difference in conclusions.] for a shot attempt. If we plug in our values for a 1 pointer with 45% chance of being made:
...
The solutions above are for an infinite series but in pickup basketball you typically play to some fixed value (e.g. 7, 11, 15, 16, 21, 25). What you actually want is the solutions for finite geometric series that are conditioned on how many points^[(actually scoring possessions)] are left in the game. When playing make-it-take-it by 1s and 2s, 1-pointers actually show a slight advantage if there are 4 or less points remaining in the game after which the small advantage shifts to 2-pointers. In general though, calculating the sums of the finite series tells the same overarching story as we told when calculating expected values with infinite series. "*Given 45% : 30% (interior : long range) shooting percentages, make-it-take-it by 1s and 2s is pretty balanced whereas make-it-take-it by 2s and 3s gives an expected points advantage to interior shooting.*" 

Therefore infinite series seem to work fine as an approximation for expected points while being simple to describe as each statement does not need to be conditioned on how many points remain in the game. See the [Finite geometric series] section in the appendix if you're interested in more detail though. 

## Summary

The two conditions that lead to similar in-game expected values for interior and long range shots are make-it-take-it 1s and 2s as well as 2s and 3s with alternating possession after scores^[The other play formats lead to imbalances that favor either long range or interior shooting.]. 
...
You might think that the expected values of different shots would naturally balance out. If you're playing by 1s and 2s with alternating possession (where deep shots are more valuable) you would think players would defend long range shots more aggressively which would lead to the shooting percentage on those shots dropping relative to interior shots. In practice though, this adjustment does not happen as much as would be expected by fully rational players. Instead, many people anchor their play style to whatever brand of basketball they are accustomed and do not fully adjust to the scoring and possession system in place. Given these tendencies, *what should you do?*
...
This means if you're playing full court (with alternating possessions after scores), push for playing by 2s and 3s to 21 (rather than the more common 1s and 2s to 15). If you're playing half court, you don't need to do anything as games are typically played make-it-take by 1s and 2s, which has mostly fine incentives^[You *could* push for 2s and 3s with alternating possession though which is a little more balanced and has the advantage of being more robust to variations in shooting percentage which I touch-on in [Other considerations].]. 

2. Arbitrage

In many cases you will not be able to influence the scoring system. Under these conditions other players not fully adapting to unbalanced values of interior or long range shots is an opportunity. For example, if the system advantages long range scoring, do your best to get your best shooter as many decent looks as possible and push your team to play good outside defense[^well-balanced-games].
...
The conclusion that "1s and 2s make-it-take-it" and "2s and 3s alternating possession" make for the most balanced games depends on my guesses for shooting percentages[^personal-problems]. 45% and 30% for interior and long range are reasonable estimates for competitive pickup games but may not apply universally[^ratios-relations]. 
...
The wider color hue range in make-it-take-it settings demonstrates how, as you deviate from balanced shooting percentage conditions, shot imbalance grows more quickly than when playing with alternating possession after scores^[This is why the 45%; 30% shooting conditions in the upper right and lower left, while being very different in terms of distance from the "perfect balance" lines are pretty similar in terms of proportion imbalanced.]. Everything above the red "perfect balance" lines (or curves in the cases of make-it-take) can be interpreted as shooting percentages where long range shooting has an expected points advantage. Everything below them represents shooting percentages where interior shooting has an advantage. For example, in games where interior shooting percentages are still ~45% but long range shooting is a bit worse, e.g. percentages in the low 20s, 1s and 2s with alternating possession after a score can actually be more balanced than 2s and 3s. However you would still be setting-up conditions for a lot of missed shots. 
...
Calculating the finite geometric series is more precise as it allows us to consider how many points (/scoring rounds) are left in the game. Equation for calculating the value of a finite geometric series:

$$
\frac{\text{(start value)} \cdot (1 - \text{rate}^n)}{(1 - \text{rate})}
$$

In our case, *n* is the number of remaining scoring possessions. However the number of remaining scoring possessions will depend on whether you're shooting interior or long-range shots. Long range shots are worth more points so there would be fewer scoring possessions until the end of the game. For example, if you're playing by 1s and 2s there will be ~half as many scoring possessions remaining if you're sinking long-range compared to interior shots^[In the curves I'm ignoring that some of these point values are impossible given the scoring systems.]. 

Taking this into account, let's look at the expected value of each shot if we range the number of points left in the game from 2 to 20 (still using our 45% and 30% shooting percentages).
...
Interior shots actually have a *slight* advantage when there's <= 4 points remaining in the game. However the values are still not *that* far apart making for a reasonably balanced game throughout.
