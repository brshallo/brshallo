---
title: 'Riddler Solutions: Palindrome debts and ambiguous absolut value
  signs'
...
This post contains solutions to FiveThirtyEight’s two riddles released 2020-02-07, [Riddler Express] and [Riddler Classic]. Code for figures and solutions can be found on my github page.
...
We could use the `combinat` package in R to generate all these combinations. 
...
One thing I needed to do was make it so our mathematical solutions, i.e.:
...
I made an equation `create_solve_expr_df()` that creates the expressions and computes the solutions.  See the [raw Rmd file](github page link) on my github to see the code^[The code isn't *the most* attractive. The dataframe set-up could be cleaner.  Also I'd like to go back and rewrite the expression part of this using `rlang` and some of the cool things you can do with manipulating environments and expressions in R... but alas... hacked this solution together by just stitching together text...] I wrote to accomplish this.
...
This led to an equal number of configurations and unique solutions -- no duplicates. Hence the fact there were duplicates in our problem was specific to the inputs of -1 to -9 not something that would result when inputting any 9 numbers into this expression. I also found this to be the case on longer expressions up to length 17 i.e. $|x_1|x_2|x_3|...|x_17|$.

## More than 9 numbers

With the above set-up you could calculate the number of configurations for any length of input. Though I found that once I started getting into problems into the 20's things take a long-time to process. See below for a chart of unique solutions from 1 to 17^[Note also that this problem requires that there be an odd number of inputs and that they all be negative.]
...
num_solutions %>% 
  ...
  geom_line(aes(y = n_configs, colour = "num config"))+
  geom_line(aes(y = n_unique, colour = "num unique"))+
  labs(title = "Number of potential interpretations grows exponentially", 
...
My set-up took inspiration from David Robinson's approach to a different [FiveThirtyEight "Riddler" problem](https://www.youtube.com/watch?v=TDzd73z8thU).
...
* Gives the number of meaningful configurations of parentheses 
    * Would still need to go and make these all 