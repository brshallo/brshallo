...
... figured I'd share what I've patched together so far `r emo::ji("smile")` (will just be looking at transformations by 2x2 matrices).

If you are not interested in walking through the R code, skip to [Visualizations].
...
`transform_df_coords()` is just matrix multiplication, but facilitates applying matrix transformations on a dataframe where each row (in specified columns) represents a coordinate point.
...
And there it is. To view a different matrix transformation, simply change the `transformation_matrix` defined above and re-run the code chunks^[Could functionalize more... or make a shiny app, or do more with, see [Problems and potential improvements] for notes...].

# Appendix

The code used to construct images within the appendix is very similar to code already shown (so can largely be skimmed over).

## Multiple matrix transformations

I love how the "Essence of Linear Algebra" series explains how matrix transformations can be thought-of / broken-down sequentially. The same visualization can be set-up here -- you just need to add-in the additional layer.
...
Notice that we see the transformatioins done sequentailly. We could also have just inputted the single (simplified) matrix transformation:
...
I have no (current) plans of fleshing this out further. (Though I think a ggplot extension -- e.g. `ggbasis`, `gglineartrans` -- or something could be cool.)  In this section I'll give a few notes regarding short-term things I'd change or fix-up (if I were to keep working on this -- maybe I'll get to a couple of these).

### Problem of squeezing during rotation

You might notice that something about the rotation transformation looks a little off. During the animation, the grid becomes artificially and temporarily squished in at some points. We can better see this by placing a circle on the interior of our grid and looking at the rotation of the exterior segments. The exterior segments of the grid *should* remain tangent to our circle at all points.
```{r}
...
However we can see this doesn't happen (the grid scrunches up and the exterior segments cut into the circle)^[I will investigate what can be fixed or open an issue. Am still new to using `gganimate` so guessing / hoping `r emo::ji("crossed_fingers")` is simple fix.].
