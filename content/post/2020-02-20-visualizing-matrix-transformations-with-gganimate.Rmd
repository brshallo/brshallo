...
Transformations that you *could* conceptualize of as rotations will be animated as linear changes to coordinates. As a more extreme example, see animation of a matrix transformation for a 180 rotation: 
...
One fix (irrespective of tweening method in `gganimate`) could be to set specific coordinates at each frame (so that the lack of a true rotation wouldn't noticable)^[Though this gets into decomposing the rotation, etc. components of the matrix transformation of interest for each frame.].
