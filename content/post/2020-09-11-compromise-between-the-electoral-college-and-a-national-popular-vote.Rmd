...
This approach would empower small states whose voters ballots would be essentially worth double that of those in large states^[https://theconversation.com/whose-votes-count-the-least-in-the-electoral-college-74280]. While this may seem horribly unfair, it only makes the existing biases in the electoral college more transparent. Also, it still represents a major improvement in representativeness over the existing system where only a ~quarter of the population’s votes make any *real* difference in the outcome of the election (specifically those votes in battleground states^[“Influence here is evidenced by campaign events and ad spending among candidates as described [here](https://www.nationalpopularvote.com/written-explanation).]). 

Such a weighted national popular vote would preserve (in part) any advantage a party has in the current electoral college. This would make associated legislation potentially more palatable and enduring across changes in state-level leadership (compared to legislation for a raw popular vote). The weighted popular vote also maintains the essence of the existing electoral college^[I.e. it still spreads out votes by states and not just across the population] -- it just replaces 'winner-take-all' with 'proportional' allocation of a state's electoral votes (a less glaring difference compared to adopting a purely national popular vote).
