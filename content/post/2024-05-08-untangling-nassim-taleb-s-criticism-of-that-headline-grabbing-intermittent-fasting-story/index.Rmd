...
I want to discuss his complaints regarding sample size in relation to the event rate observed in this study^[Which he suggests is all that is needed to dismiss the results, even if you ignore potential issues in setup: https://x.com/nntaleb/status/1770467069454073997]

While there is indeed a need for a larger sample when there is a low event rate, it's not on its own a justification for throwing-out a small p-value on a post-hoc basis. The p-value represents (taking into account the sample size and the underlying variability / event rate) how likely it is you would get the observed results by random^[p-value of 0.006 suggests these results are unlikely to have occurred by chance alone.]. The need for a large sample size in the case of rare events often has more to do with increasing the power of your study to accommodate for the difficulty of detecting rare events^[related to type 2 errors: ~failure to detect an effect when one truly exists -- a false negative] compared to the problem of getting a false positive finding where an effect actually does *not* exist^[Which relates to a type 1 error]. Which I articulate in a [reply](https://x.com/brshallo/status/1772422125069963359) on his thread[^study-design]: 
