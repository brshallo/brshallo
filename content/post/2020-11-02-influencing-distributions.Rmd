...
If you have a mechanism available for changing behavior, before applying it you should consider the existing distribution and the potential changes you will cause. In this post, I will use incentives for sales representatives in pricing as my example.
...
data_distribution %>% 
  ...
  group_by(incentive) %>% 
  summarise(proportion_converted = sum(density_convert) %>% round(3)) %>% 
  knitr::kable()
```
