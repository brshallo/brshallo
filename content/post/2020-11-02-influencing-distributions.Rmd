...
If you have a mechanism available for changing behavior, before applying it you should consider the existing distribution and the potential changes you will cause. In this post I will use incentives for sales representatives in pricing as my example. All charts are generated using R code (see [Rmarkdown document](https://github.com/brshallo/brshallo/blob/master/content/post/2020-11-02-influencing-distributions.Rmd) on github for code).

For instance, if you have a lever that pushes prices from low to high, using the lever to influence the prices adjacent to the right of the largest parts of the distribution will (likely) make the biggest impact on raising the average price attained. If the starting distribution is normal, this means incentives applied near the lower prices (the tail of the distribution) may have the smallest impact.
...
*Y axis is large in order to be consistent with future, similar charts*
...
* *Take a moment to hypotheisze what the new distribution will look like after incentives are applied*. 

The resulting distribution after applying incentives is likely to depend on:
...
1. How this influence degrades the farther away the starting position of a deal is from a newly incentivized cut-point.
...
Gray bars in the chart below indicate where, in the original distribution, movement will occur.
```{r}
data_distribution %>% 
  mutate(quartile = (cum_dens) %/% 0.2500001 + 1,
         quartile = as.factor(quartile)) %>% 
...
The greatest proportion of deals were moved from orange to yellow and from yellow to blue. Pink to orange had the lowest rate of conversion.
```{r}
data_distribution %>% 
  mutate(quartile = (cum_dens) %/% 0.2500001 + 1,
         quartile = as.factor(quartile)) %>% 
...
**Incentives Make the Biggest Difference When Nearer to the Largest Parts of the Distribution Suseptible to Change**

Because these incentives slide deals from lower prices to higher prices, those cut-points that are *just above* the largest parts of the distribution have the biggest impacts on the post-incentivized distribution. For a normal distribution, such as this one, that means incentives just to the right of the first quartile have the smallest impact. (Importantly, this assumes suseptibility to change is evenly distributed across the starting distribution.)
